...
export class RegistrationComponent implements OnInit {
  ...
  registerForm = this.formBuilder.group({
    firstName:'',
    lastName:'',
    userName: ['',[Validators.required,Validators.pattern(/^[A-Za-z][A-Za-z]*[A-Za-z]{3,25}$/)]],
  ...
  register() {
    ...
    this.registrationService.registerUser(userObj).subscribe(
      (success)=>{
        ...
        this.showNotification('success', "User is successfully Registered!!!").then(res=>this.router.navigate(['/login']));
