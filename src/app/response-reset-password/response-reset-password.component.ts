...
export class ResponseResetPasswordComponent implements OnInit {
  ...
  VerifyToken(){
    this.responseResetService.ValidPasswordToken({resetToken : this.resetToken}).subscribe(
      (success)=>{
        console.log(success);
  ...
  ResetPassword(form){
    ...
    this.responseResetService.newPassword(this.ResponseResetForm.value).subscribe(
      (success)=>{
        this.ResponseResetForm.reset();
        this.spinner.hide();
        this.showNotification('success', "Password reset successfully!!!").then(res=>this.router.navigate(['/login']));
      },
      (error)=>{
